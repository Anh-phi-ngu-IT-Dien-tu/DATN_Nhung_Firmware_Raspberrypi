...
uint8_t tx_buffer2[12];
uint8_t rx_buffer2[8];

uint8_t final_rx_buffer2[8];
uint8_t uart3_rx_buffer[8];
uint8_t uart3_tx_buffer[8];

/*interrupt control variables*/
int8_t uart3_allow_tx;
int8_t uart3_rx_check;
int8_t uart2_allow_tx;
int8_t uart2_rx_check;
int8_t timer2_check;
#if Visualize_Measurement==1
int8_t i=0;
#endif
...
Motor_measurement_command_typedef M1;
Motor_measurement_command_typedef M2;
PIDControllers_Typedef pidM1;
PIDControllers_Typedef pidM2;
...
void Deal_Timer2()
{
	...
	if(i==5)
	{
		sprintf((char *)tx_buffer2,"%.4f/%.4f/%d/%d/%d/%d\n",M1.rpm_measurement,M2.rpm_measurement,M1.current_cnt,M1.previous_cnt,M2.current_cnt,M2.previous_cnt);
